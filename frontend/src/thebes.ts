...
export const BACKEND_CANISTER_ID = 207968450065829;
...
const TYPE_NAT_SLEB = 0x7d; // sleb128(-3), the `nat` type code
...
// ─── multi-argument primitive encoding (added for notes-app) ─────────
//
// Candid layout for N primitive args: all N type codes first, then all
// N values in the same order. No type-table entries needed — primitives
// only, same trick encodeText already uses for the single-arg case.

type ArgValue =
  | { kind: "text"; value: string }
  | { kind: "nat"; value: bigint | number };

function encodeArgs(values: ArgValue[]): string {
  const bytes: number[] = [...MAGIC, 0, values.length];
  for (const v of values) {
    bytes.push(v.kind === "text" ? TYPE_TEXT_SLEB : TYPE_NAT_SLEB);
  }
  for (const v of values) {
    if (v.kind === "text") {
      const utf8 = new TextEncoder().encode(v.value);
      bytes.push(...uleb(BigInt(utf8.length)), ...utf8);
    } else {
      bytes.push(...uleb(BigInt(v.value)));
    }
  }
  return bytesToHex(new Uint8Array(bytes));
}

/// Encode a single `(nat)` argument — e.g. remove(id).
export function encodeNat(n: bigint | number): string {
  return encodeArgs([{ kind: "nat", value: n }]);
}

/// Encode a `(text, text)` argument pair — e.g. add(title, body).
export function encodeTextText(a: string, b: string): string {
  return encodeArgs([
    { kind: "text", value: a },
    { kind: "text", value: b },
  ]);
...
/// Encode a `(nat, text, text)` argument triple — e.g. edit(id, title, body).
export function encodeNatTextText(
  id: bigint | number,
  a: string,
  b: string
): string {
...
/// plus bool (0x7e), added for edit()/remove() which return Bool.
export function decodeReply(hex: string): string | bigint | boolean {
...
export async function query(method: string, argHex: string): Promise<string | bigint | boolean> {
...
export async function call(method: string, argHex: string): Promise<string | bigint | boolean> {
...
async function pollReceipt(hashHex: string): Promise<string | bigint | boolean> {
